...
enum HistoryTimeScope { pastAndToday, upcoming, all }
...
class FilterHistoryUseCase {
  static List<DoseOccurrenceModel> filter({
    ...
    return occurrences.where((occ) {
        ...
        if (query.isNotEmpty) {
          final nameMatch = occ.medicineNameSnapshot.toLowerCase().contains(
            query,
          );
          final strengthMatch = occ.medicineStrengthSnapshot
              .toLowerCase()
              .contains(query);
          final foodMatch = occ.foodInstructionSnapshot.toLowerCase().contains(
            query,
          );
